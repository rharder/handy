...
def extract_vba_from_office_files(args):
    ...
    for infilename in args.files:
        for filename in glob.glob(infilename):
            if args.sort:
                ...
                firstname = filename[ filename.rfind("_", 0, filename.find("@")) + 4 : filename.rfind(".", 0, filename.find("@")) ].title()
    ...
    with open(args.outfile, "w") as outfile:
        ...
        print("""<script src="https://cdn.rawgit.com/google/code-prettify/master/loader/run_prettify.js?lang=vb"></script>""", file=outfile)
        ...
        print("""<div style="width: 67%; -webkit-column-count: {0}; -moz-column-count: {0}; column-count: {0};">""".format(3 if args.sort else 1), file=outfile)
        ...
        print("""<h3><a href="javascript:collapse_all()">Collapse All</a> &ndash; <a href="javascript:expand_all()">Expand All</a></h3>""", file=outfile)
        for key in sorted(filenames):
            filename = filenames[ key ]
            a = hashlib.sha256(filename.encode()).hexdigest()
    ...
    for (filename, stream_path, vba_filename, vba_code) in vba_parser.extract_macros():
        ...
        if args.modules is None or stream_path[ stream_path.find("/") + 1: ] in args.modules:
            if args.sort:
                ...
                print("""<h3 style="margin-left: 20px">{}: {}</h3>""".format(html.escape(office_filename), html.escape(stream_path)), file=outfile)

...
from handy.bindable_variable import Var, BindableDict
...
class NetworkMemory(BindableDict):
    ...
    def set(self, name, new_val, force_notify=False, timestamp=None):
...
class NetworkMemoryVar(Var):
    def __init__(self, local_addr, remote_addr, loop=None, **kwargs):
    ...
    def datagram_received(self, data, addr):
        ...
        if "name" in msg:
            ...
            self.log.debug("Updating with network data: {} = {}".format(name, value))
            self.set(value)
            # print("self._value:", self.value)


class NetworkMemoryVar1(Var):
